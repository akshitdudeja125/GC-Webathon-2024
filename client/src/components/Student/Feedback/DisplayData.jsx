...
const DisplayData = (props) => {
    ...
    const submitHandler = () => {
        const getting = async ()=>{
            try{
                ...
                const res=await axios.post("http://localhost:3002/api/student/submitCourseFeedback", update);
                console.log(res)
  ...
  return (
    <div>
      {props.obj["courses"].map((res, idx) => (
        code=res["courseId"].split("_")[0],
        <div key={idx} className={"grid grid-cols-12 hover:scale-105 transition-all duration-150 grid-cols-7"}>
